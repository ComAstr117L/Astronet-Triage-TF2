...
def _build_metrics(labels, predictions, weights, batch_losses):
  ...
  if binary_classification:
    ...
    predicted_labels = tf.cast(
        tf.greater(predictions, 0.5), dtype=tf.int32, name="predicted_labels")
  else:
  ...
  with tf.compat.v1.variable_scope("metrics"):
    ...
    is_correct = weights * tf.cast(tf.equal(labels, predicted_labels), dtype=tf.float32)
    update_num_correct = tf.compat.v1.assign_add(num_correct, tf.reduce_sum(input_tensor=is_correct))
    ...
    if binary_classification:
      ...
      def _count_condition(name, labels_value, predicted_value):
        ...
        update_op = tf.compat.v1.assign_add(count, tf.reduce_sum(input_tensor=weights * is_equal))
