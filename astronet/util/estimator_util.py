...
def evaluate(estimator, input_fn, eval_steps=None, eval_name="val"):
  ...
  tf.compat.v1.logging.info("Starting evaluation on checkpoint %s", latest_checkpoint)
  try:
    ...
    tf.compat.v1.logging.info("Checkpoint %s no longer exists, skipping evaluation",
                    latest_checkpoint)
...
def continuous_eval(estimator,
  ...
  Yields:
    ...
    the time the TPU worker initializes.
  """
  for _ in tf.train.checkpoints_iterator(estimator.model_dir):
...
def continuous_train_and_eval(estimator,
  ...
  while True:
    ...
    tf.compat.v1.logging.info("Starting training at global step %d", global_step)
